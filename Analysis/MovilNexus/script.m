...
trainingRssi = [];
trainingLabels = [];
prefix = '/home/jeslev/Dropbox/Bluetooth/ResultadosJesus/MovilNexus/Tx_0x05/Tx05_';
for it=2:3
    filename = strcat(prefix,int2str(it),'.csv');
    data = csvread(filename,2,1);
    [Tlabel, Trssi] = formatData(data);
    trainingRssi = [trainingRssi; Trssi];
    trainingLabels = [trainingLabels; Tlabel];
    %matrixCSV = csvread(filename,1,0);
end


%% Obtener media de RSSI por sector
%beacon = 1; % [7 8 9 10 11]
%medidas = [];
%for it=1:15
...
data = csvread(filename,2,1);
[validationLabel, validationRssi] = formatData(data);
