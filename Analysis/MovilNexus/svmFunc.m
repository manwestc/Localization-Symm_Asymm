function [ completeErrorMeasure, completeFinalResultsDistance, ...
        correctPositionFrequencies, totalPositionFrequencies ] = svmFunc(  trainingRssi, validationRssi,trainingLabels, validationLabel, reps )
...
for rep=1:reps
    ...
    % Train the SVM
    model = svmtrain(trainingLabels, trainingRssi, '-s 0 -t 0');
    % EXPONENCIAL model = svmtrain(trainingLabels, trainingRssi, '-s 0 -t 2');
    %model = svmtrain(trainingLabels, trainingRssi, '-s 0 -t 1 -d 2');
    %model = svmtrain(trainingLabels, trainingRssi, '-s 0 -t 1 -d 3');
